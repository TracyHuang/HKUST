...

{
    ...
    // for each thread first got to know the index of the key1 element it is responsible for
	...
	int current_key = key1[index];
	bool found = false;
	for (int i = 0; i < N2; i ++) {
		if (current_key == key2[i]) {
			result[index] = i;
			found = true;
			break;
		}
...
__global__ void equiJoinTiled(int *key1, float *value1, int *key2, float *value2,int N1,int N2,int *result)
{
	...
	/* fill in your code here */
	// each thread still takes care of one element
	// the difference with above one is that in each iteration we load TILE_WIDTH elements into a shared array and compare them
	...
	for (int i = 0; i < ceil(double(N2) / TILE_WIDTH); i ++) {
		s_key[threadIdx.x] = key2[i * TILE_WIDTH + threadIdx.x];
		__syncthreads();

		if (need_continue) {
			for (int j = 0; j < TILE_WIDTH; j ++) {
				if (s_key[j] == current_key) {
					result[index] = i * TILE_WIDTH + j;
					need_continue = false;
					break;
				}
			}
		...
		if((i == ceil(double(N1) / TILE_WIDTH) - 1) && need_continue) {
			result[index] = -1;
		}
	}
